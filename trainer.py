...
import os
from generator_image import generator_image_to_emb
import random
from discriminator_em import discriminator_emb
class Trainer(object):
    def __init__(self, type, dataset, split, lr, diter, vis_screen, save_path, l1_coef, l2_coef, pre_trained_gen, pre_trained_disc, batch_size, num_workers, epochs):
        ...
        if pre_trained_gen:
            ...
            self.generator2.load_state_dict(torch.load(pre_trained_gen))
        else:
            self.generator1.apply(Utils.weights_init)
    ...
    def train(self, cls=False):
        ...
        if self.type == 'wgan':
            ...
            self._train_gan(cls)
        elif self.type == 'emb_gan':
            self._train_emb_gan(cls)
    ...
    def _train_gan(self, cls):
        ...
        for epoch in range(self.num_epochs):
            for sample in self.data_loader:
                ...
                if cls:
                    outputs, _ = self.discriminator1(wrong_images, right_embed)
    ...
    def _train_emb_gan(self, cls):
        ...
        for epoch in range(self.num_epochs):
            for sample in self.data_loader:
                ...
                right_embed = sample['right_embed']
                wrong_images = sample['wrong_images']
                wrong_embed=random.choice(sample['right_embed'])
                
                right_images = Variable(right_images.float()).cuda()
                right_embed = Variable(right_embed.float()).cuda()
                wrong_images = Variable(wrong_images.float()).cuda()
                wrong_embed=Variable(wrong_embed.float()).cuda()
                
                
                real_labels = torch.ones(right_embed.size(0))
                fake_labels = torch.zeros(right_embed.size(0))
                ...
                noise = Variable(torch.randn(right_embed.size(0), 100)).cuda()
                noise = noise.view(noise.size(0), 100, 1, 1)
                out_img=self.generator1(right_embed,noise)
                ...
                # g_loss +=  self.l2_coef * l2_loss(activation_fake, activation_real.detach()) \
                #           + self.l1_coef * l1_loss(fake_embed, right_embed)
